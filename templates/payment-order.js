...
const PaymentOrderTemplate = {
  ...
  renderDocumentDate(dateStr) {
    ...
    return `
      <div class="doc-preview__date-line">
        <span class="doc-preview__date-row">
          <span class="doc-preview__date-prefix">中華民國</span>
          <span class="doc-preview__date-part">
  ...
  renderPartySection(label, party) {
    ...
    if (this.hasValue(party.idNumber)) {
      lines.push(`<p class="doc-preview__line">身分證字號：${this.escapeHtml(String(party.idNumber).trim())}</p>`);
    }

    if (party.birthDate) {
      lines.push(`<p class="doc-preview__line">出生年月日：${this.formatRocDate(party.birthDate)}</p>`);
    }

    if (this.hasValue(party.address)) {
      lines.push(`<p class="doc-preview__line">住、居所：${this.escapeHtml(String(party.address).trim())}</p>`);
    }

    if (this.hasValue(party.phone)) {
      lines.push(`<p class="doc-preview__line">電　　話：${this.escapeHtml(String(party.phone).trim())}</p>`);
    }

    return `<div class="doc-preview__block">${lines.join('')}</div>`;
  },

  renderClaimItem(claim) {
    const amount = this.formatAmount(claim.amount);
    const hasRate = claim.interestRate !== '' && claim.interestRate != null;
    const hasDate = claim.interestStartDate;
  ...
  render(data) {
    ...
    const reason = claim.reason && String(claim.reason).trim()
    ...
    return `
      <div class="doc-preview">
        ...
        <div class="doc-preview__block">
          ...
          <p class="doc-preview__line doc-preview__line--indent doc-preview__reason">${reason}</p>
        </div>
