...
const PromissoryNoteTemplate = {
  ...
  formatAmount(amount) {
    ...
    return Number.isNaN(number) ? '　　　　' : number.toLocaleString('zh-TW');
  },
  ...
  formatCourt(court) {
    const value = court && String(court).trim()
      ? this.escapeHtml(String(court).trim())
      : '　　　　　　　　　';
    return value;
  },
  ...
  renderPartySection(label, party = {}) {
    ...
    if (this.hasValue(party.idNumber)) {
      lines.push(`<p class="doc-preview__line">身分證字號：${this.val(party.idNumber)}</p>`);
    }
    if (this.hasValue(party.address)) {
      lines.push(`<p class="doc-preview__line">住、居所：${this.val(party.address)}</p>`);
    }
    if (this.hasValue(party.phone)) {
      lines.push(`<p class="doc-preview__line">電　　話：${this.val(party.phone)}</p>`);
    }
  ...
  render(data = {}) {
    ...
    const court = this.formatCourt(claim.court);
    ...
    return `
      <div class="doc-preview">
        ...
        <div class="doc-preview__block">
          ...
          <div class="doc-preview__reason">相對人簽發上開本票交付聲請人，詎屆期提示未獲付款，爰依票據法第123條規定，聲請鈞院裁定准予強制執行。</div>
        </div>
        ...
        <div class="doc-preview__block doc-preview__attachments">
          ...
          <p class="doc-preview__line doc-preview__line--indent">一、本票正本及影本各乙份（正本請確認後擲回）。</p>
        </div>
      </div>
    `;
  }
